...
\usepackage{multicol}
\pagestyle{head}
\header{AM 111 Class 12}{}{Numerical differentiation, p.\thepage}
\runningheadrule
\headrule
\usepackage{siunitx}
...
\begin{parts}
\item Write pseudocode for finding \texttt{error[k]}, the error associated with estimating $f'(x)$ (without using extrapolation) for $h =$\texttt{hvals[k]}.
\vspace{1in}
