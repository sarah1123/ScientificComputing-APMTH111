...
\usepackage{multicol}
\pagestyle{head}
\header{AM 111 Class 18}{}{Initial value problems: differential equations, p.\thepage}
\runningheadrule
\headrule
\usepackage{siunitx}
...
\vspace{0.5in}
\item The slope $dy/dx$ will be tangent to a solution curve $y(x)$ at every point on the curve.  Use this fact about tangencts to sketch a solution curve that starts at $y(-2) = 1$ for each of the slope fields below.
\end{parts}
...
\item For this differential equation we are able to find an exact.  Substitute into $dy/dt - cy$ to show that $y(t) = y_0e^{ct}$ is an exact solution to this IVP.
\vspace{1in}
...
\begin{enumerate}[resume=classQ]
    \item (improved Euler) In Euler's method, $w_{k+1} = w_k + hf(t_k, w_k)$.
    \begin{parts}
